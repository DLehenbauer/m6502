...
// Top-level RTL shared by Klaus Dormann's 6502 test suite. The testbench
// (tb_mcu_klaus.cpp) loads the appropriate .bin into bram via --public-flat-rw
...
// A feedback register at $BFFC drives IRQ / NMI from inside the test program.
//
//   | Bit | Signal  | Value 0             | Value 1            |
//   |-----|---------|---------------------|--------------------|
//   |  0  | i_irq_n | deasserted (high)   | asserted (low)     |
//   |  1  | i_nmi_n | deasserted (high)   | asserted (low)     |
//
...
// bram contents are loaded from C++ (see tb_mcu_klaus.cpp).
bram bram (
    .i_clk(i_clk),
    .i_phi2(cpu_phi2),
    .i_addr(bus_addr),
