...
always_comb begin
    ...
    if (i_handle_irq || i_init) begin
        ...
        // NMOS illegal JAM/KIL opcodes. After the opcode fetch and the
        // operand dummy read the core enters the JAM state and stays there:
        // SYNC never re-asserts and the bus reads the dead-cycle address
        // pattern until RES. Listed before any other arm so the type
        // patterns below cannot claim these opcodes.
